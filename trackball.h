...
struct trackball
{
	...
	float t0=0;					// 함수 불린 시점.
	float speed=0.1f;			//camer의 이동 속도 . 아마 탱크의 이동속도랑 같아야될듯?


	//*********아래 begin ALL end ALL 함수들 주석 지우면 wasd left right 동시에 누르기 차단함 ************//
	void begin_ALL() {/*
		b_W = true;
		b_A = true;
		b_S = true;
...
inline mat4 trackball::update_panning(vec2 m) const
{
	...
	cam.eye = cam.eye+ up * p1.y + side*p1.x;
	cam.at = cam.at+ up*p1.y + side*p1.x;
...
inline mat4 trackball::update_zooming(vec2 m) const
{
	...
	vec3 towards = (cam.at- eye0).normalize();
	
	cam.eye = cam.eye+ towards * p1.y * 10;
...
inline mat4 trackball::update_W(vec2 m) const
{
	...
	cam.eye = cam.eye+ towards * (t - t0) * speed;
...
inline mat4 trackball::update_A(vec2 m) const
{
	...
	cam.eye = cam.eye- side * (t-t0) * speed;
	cam.at = cam.at- side * (t - t0) * speed;
